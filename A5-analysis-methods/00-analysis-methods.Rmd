...
[@Lun2018-lb] available in the DropletUtils package (v`r versions$DropletUtils`)
...
droplets. The droplets selected by either the Cell Ranger v3 method or emptyDrops
were kept giving a dataset with `r params$pre$n_cells` cells. Gene annotation
information was added from BioMart [@Smedley2015-an] using the biomaRt package
(v`r versions$biomaRt`) [@Durinck2005-zb] and cells were assigned cell cycle
scores using the cyclone [@Scialdone2015-rp] function in the scran package
(v`r versions$scran`) [@Lun2016-mq].

### Alevin

An alternative quantification was performed using the Alevin method
[@Srivastava2018-fy] available as part of the salmon software package
...
Partition-based graph abstraction (PAGA) [@Wolf2017-vq] was performed using the
