...
the knee point of the cumulative fraction of counts with respect to increasing
total count [@Macosko2015-rl]. While these methods effectively select most real
cells they risk overlooking cells that have a lower RNA content than the
majority of cells in the sample. The emptyDrops method [@Lun2019-wp] in the
DropletUtils package provides an alternative approach (Figure
\@ref(fig:droplet-selection)). A threshold on total counts is selected below
...
version of the emptyDrops procedure where the original selection method is
substituted for the knee point threshold.
...
package provide different implementations of what is largely the same
...
real cells or the algorithm has been too permissive and selected droplets that
are actually empty. In the following section I will perform further quality
control to remove low quality cells so at this stage I chose to keep cells
identified by either the emptyDrops or Cell Ranger methods. Of the
...
have had to be adapted for scRNA-seq data. Alevin [@Srivastava2019-ce] is the
...
data they were given rather than the selection methods themselves. Alevin (and
Cell Ranger v3) also processes the three samples separately while I performed
