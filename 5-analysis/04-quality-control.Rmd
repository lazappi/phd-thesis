...
adding annotations and visualisations while scran can be used for various other
...
added mitochondrial genes as a feature control set as the amount of counts
...
scores in order to assign a phase to each cell. Pre-computing a range of
metrics is a valuable feature of scater as they are immediately available for
...
is made easy by the isOutlier function in scater. The first thing I filter on
is the number of counts per cell applying thresholds to the total number of
counts per cell and the total number of features that are expressed (Figure
...
by combining randomly selected cells. The density of simulated doublets in the
neighbourhood of each cell in PCA space is then used to calculate a doublet
score to which a threshold can be applied (Figure \@ref(fig:qc-thresholds)C).
For this threshold I have chosen a value that gives a number of doublets
roughly equal to the expected number of the cell capture technology (around one
percent for 10x Chromium).
...
(ref:cap-qc-thresholds) Thresholds used for quality control. (A) Scatter plot of total counts per cell against number of features expressed. Thresholds for selecting high-quality cells show by purple (features) and green (counts) dotted lines. Points are coloured according to whether cells were kept (blue) or removed (pink). (B) Histogram of percentage of counts assigned to mitochondrial genes with threshold shown in pink. (C) Histogram of doublet score genes with threshold shown in pink. (D) PCA plot based on technical factors. Cells are coloured according to whether they were removed by both the PCA outlier method and manual filtering (orange) or just manual filtering (blue).
...
two high-quality cells. After quality control the dataset had
`r params$qc$n_cells` cells and `r params$qc$n_genes` genes with median of
`r params$qc$median_counts` counts per cell and a median of
`r params$qc$median_genes` expressed.
...
would be more appropriate. That is not the case here as can be seen from the
