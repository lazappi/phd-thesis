...
cells in a cluster and all other cells in the dataset, then to inspect the
...
RNA-seq or standard statistical tests [@Soneson2018-fb; @Jaakkola2016-ko].
...
example a method such as ZINB-WaVE can be used to calculate weights that can be
added to the edgeR model to correct for zero inflation [@Van_den_Berge2018-ip].
In this case I decided the edgeR fit was sufficiently good and these kinds of
...
that are highly expressed. One we have a fitted negative binomial model we can
...
foldchanges and these are consistent with the overlaps between the published
...
some organoid cells that clustered with immune cells in human foetal kidney in
our previous analysis and there is a plausible pathway by which they could
